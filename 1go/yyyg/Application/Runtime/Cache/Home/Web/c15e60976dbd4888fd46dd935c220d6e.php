...
<html lang="zh-CN">
  <head>
    <meta charset="utf-8">
    <title>用户中心 - <?php echo ($web_title); ?></title>
	...
	<link href="<?php echo ($web_tplpath); ?>css/user.css" rel="stylesheet" />
...
<div class="top-back">
	...
	<div class="navbar category">
		...
		<div class="top-backs">
	  	<div class="container">
			<!-- 所在位置 - 开始 -->
			<div class="w-dir"><a href="<?php echo U('index/index');?>">首页</a> &gt; 个人中心</div>
			<!-- 所在位置 - 结束 -->
			...
			<div class="user-info">
				...
				<div class="user-data">
					<ul>
						<li id="setname" data-uid="<?php echo (UID); ?>">
							<h3><?php echo ($user["nickname"]); ?></h3><i class="user-data-inputs"><input type="text" value="<?php echo ($user["nickname"]); ?>"></i> <b class="useredit-name"><a href="javascript:void(0)" url="<?php echo U('user/set_username');?>" class="a3">编辑</a></b>
						</li>
						<li>
							帐号：<?php echo ($user["username"]); ?> <b class="password"><a href="#" class="a3">修改密码</a></b>
						</li>
						<!-- <li>
							绑定微信：暂无绑定 <b><a href="#" class="a3 weixinb">立即绑定</a></b>
						</li> -->
						<li>
							绑定手机：<?php if(!empty($user['points'])): ?>已绑定<?php else: ?>暂无绑定 <b><a href="#" class="a3 phone">立即绑定</a></b><?php endif; ?>
						</li>
						<li>
							账户余额：<span class="red"><?php echo ($user["black"]); ?></span> <?php echo ($web_currency); ?>
						</li>
						<li>
							<a href="<?php echo U('Pay/recharge');?>" class="btn btn-pink">立即充值</a>
						</li>
					</ul>
				</div>
			</div>
		...
		<div class="product-nav-t">
			...
			<div class="product-content">
				...
				<div class="tabs-panel-item">	
					<div class="user-duobao-list">
						...
						<table class="w-table">
					        ...
					        <tbody id='records' url="<?php echo U('user/records?state='.I('state'));?>">
				...
				<div class="tabs-panel-item t-left lotteryScrollbar">
					<div class="goodsList lottery-list" url="<?php echo U('user/lottery');?>">
						{{if {{shop_express_no}}}}
				...
				<div class="tabs-panel-item shareScrollbar">
					<div class="detail-shareList">
						<ul class="share-list" url="<?php echo U('user/displays/?uid='.$user['id']);?>">
					...
					<div class="user-duobao-list">
						...
						<div class="tabs-nav-item">
							...
							<div class="input-group">
								<p>1元就能买IPhone 6哦，快去看看吧！<?php echo ($web_url); ?>?id=<?php echo ($user['id']); ?></p>
							...
							<div class="inviteScrollbar">
								<table class="w-table">
							        ...
							        <tbody>
							        	<tr id='invite' url="<?php echo U('user/invite');?>">
						...
						<div class="tabs-nav-item" style="display: none">
							...
							<div class="labers-yellow mt15">
								<span class="mr15">累计收入：<span class="red"><?php echo ($user['brokerage']+$user['withdraw_brokerage']); ?></span>元</span>
								<span class="mr15">累计(提现/充值)：<span class="red"><?php echo ($user['withdraw_brokerage']); ?></span>元</span>
								<span class="mr15">佣金余额：<span class="red"><?php echo ($user['brokerage']); ?></span>元</span>
							...
							<div class="commissionScrollbar">
								<table class="w-table">
							        ...
							        <tbody>
							        	<tr id='commission' url="<?php echo U('user/commission');?>">
						...
						<div class="tabs-nav-item" style="display: none">
							...
							<table class="w-table address-tr">
								<form action="<?php echo U('/user/mention');?>" class="form-mention" method="post">
						        <tbody>
						        	<tr>
						...
						<div class="tabs-nav-item" style="display: none">
							...
							<div class="mention_listScrollbar">
								<table class="w-table">
							        ...
							        <tbody>
							        	<tr id='mention_list' url="<?php echo U('user/mention_list');?>">
							        		<td class="col1">{{shop_time}}</td>
				...
				<div class="tabs-panel-item">
					<div class="user-duobao-list">
						...
						<table class="w-table">
					        ...
					        <tbody id='recharge' url="<?php echo U('user/recharge_list');?>">
					        </tbody>
					    </table>
						</div>
					</div>
  	...
  	<div class="back-black mentiontc" style="display: none;">
  		<form method="post" action="<?php echo U('/user/mention');?>" class="form-mention-tc">
  		<div class="more-number">
  			<div class="monu-heard">
  	...
  	<div class="back-black rechargetc" style="display: none;">
  		<form method="post" action="<?php echo U('/user/mention');?>" class="form-recharge-tc">
  		<div class="more-number">
  			<div class="monu-heard">
  			...
  			<div class="monu-body">
  				<label><span>佣金金额：</span><span class="red"><?php echo ($user['brokerage']); ?></span>元</label>
	...
	<script src="<?php echo ($web_tplpath); ?>js/cityselect/jquery.cityselect.js"></script>
		...
		$(document).ready(function(){
		    ...
		    $("#sendcode").click(function(event) {
		    	...
		    	if(false == pattern.test($('[name="phone"]').val())){
			        ...
			        $.get("<?php echo U('public/sendcode',array('type'=>'bindcode'));?>",{phone:$('[name="phone"]').val()}).success(function(data){
			...
			$(".close").click(function(){
				if(uploadheads){
					$.get('<?php echo U('picture/removeup');?>');
				}
				$(".user-face-show").hide();
			});
			//修改密码
			...
			$('#address').on('click','.edit',function(){
				...
				$(this).parent().siblings().each(function(index){
					...
					if(index==2){
						...
						$("#city").citySelect({
							url: "<?php echo ($web_tplpath); ?>js/cityselect/city.min.js",
				          	prov: strs[0],
				          	city: strs[1],
				          	dist: strs[2]
				        });
